...
export async function createArchivist(username: string): Promise<Archivist> {
    ...
    if (await archivistModel.findById(username)) throw new Error("Archivist already exists");
...
export async function renameArchivist(username: string, newName: string): Promise<void> {
    ...
    if (await archivistModel.findById(newName)) throw new Error("New username already exists");
